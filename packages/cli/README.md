...
npx @madeui/cli list
npx @madeui/cli diff        # which installed files differ from the registry
```
...
`components/ui/` — the source is yours to edit. `diff` compares what you
installed with the registry's current version (`--json` for agents and
scripts).
