...
Two value-format rules keep files installable through the shadcn CLI (its
transformer rewrites some CSS-like strings): use hex-alpha colors
(`#00000080`, never `rgba(...)`) and no lone `0` spread in shadow shorthands.
