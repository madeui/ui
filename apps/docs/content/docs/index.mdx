...
An open-source UI library you own the source of — components are copied into
your project as editable files, not installed as a package — built on Base UI
(headless behavior and accessibility) and StyleX (compile-time atomic CSS).
...
design tokens; `add` copies components into `components/ui/`. See
[Installation](/docs/installation) for requirements and the manual setup.
