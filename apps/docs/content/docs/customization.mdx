...
### Partial themes compose

A theme doesn't have to override every token. A partial theme that swaps only
the brand tokens composes with a full one — later themes win per token, so an
accent layers cleanly on top of dark mode:
...
```tsx
// Dark mode with your brand color:
const { className } = stylex.props(darkTheme, brandTheme);
document.documentElement.className = className ?? '';
```

madeui's own palette is neutral by default — `primary` is near-black ink — so
the accent is yours to pick rather than one you have to override.
