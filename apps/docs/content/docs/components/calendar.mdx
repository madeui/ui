...
Every slot is styled through react-day-picker's `classNames` prop with StyleX classes; your own `classNames` are merged last, so a key you pass replaces ours for that slot.
